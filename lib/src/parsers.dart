...
List<Agency> parse_agencies(String responseXml) {
	...
	var agencies = document.rootElement.findElements('agency')
			.map((node) {
...
RouteList parse_route_list(String responseXml) {
	var document = xml.parse(responseXml);
	var routes = document.rootElement.findElements('route')
		.map((node) {
			var tag = attr_to_string("tag", node);
			var title = attr_to_string("title", node);
			var shortTitle = attr_to_string("shortTitle", node);
...
List<Predictions> parse_predictions(String responseXml) {
	var document = xml.parse(responseXml);
	var predictions = document.rootElement.findElements('predictions')
...
Route _parse_route(xml.XmlElement node) {
	...
	var stops = node.findElements('stop')
		.map(_parse_stop)
		.toList();

	var paths = node.findElements('path')
		.map((pathNode) {
	...
	return new Route(tag, title, shortTitle, colorCode,
			oppositeColorCode, stops, paths, latMin,
...
Predictions _parse_predictions(xml.XmlElement predsNode) {
	...
	List<Prediction> predictions = predsNode.findElements("direction")
		.map((dir) {
			var dirTitle = attr_to_string("title", dir);
			return dir.findElements("prediction")
				.map((pred) => _parse_prediction(pred, dirTitle));
		})
		.expand((p) => p)
		.toList();

	List<String> messages = predsNode.findElements("message")
		.map((msgNode) => msgNode.text)
		.toList();

	return new Predictions(agencyTitle, routeTag, routeTitle, stopTag,
			stopTitle, dirTitleBecauseNoPredictions, predictions, messages);
}
