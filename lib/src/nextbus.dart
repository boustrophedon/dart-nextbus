...
class Route {
  ...
  Route(
      ...
      num lonMin,
      num lonMax)
      : this.extents = new Rectangle.fromPoints(
            new Point(lonMax, latMax), new Point(lonMin, latMin));
...
class Predictions {
  ...
  Predictions(
      ...
      this.stopTag,
      this.stopTitle,
      this.dirTitleBecauseNoPredictions,
      this.predictions,
      this.messages);

  /// Returns predictions for all buses stopping at every stop on `Route route`.
  static Future<List<Predictions>> request_route_predictions(
      String agency, Route route) {
    Map<String, dynamic> params = {'a': agency};
    var stopsStrings =
        route.stops.map((stop) => route.tag + "|" + stop.tag).toList();
    ...
    return http.get(uri).then((r) {
      ...
      HashMap<String, Predictions> pmap = new HashMap.fromIterable(preds,
          key: (p) => p.stopTag, value: (p) => p);
      return route.stops.map((stop) => pmap[stop.tag]).toList();
    });
  }

  /// Returns predictions for all buses on the route with the given
  /// `routeTag` stopping at the stop with the given `stopTag`.
  static Future<Predictions> request_stop_predictions(
      String agency, String routeTag, String stopTag) {
  ...
  static Future<List<Predictions>> request_stop_predictions_all(
      String agency, int stopId) {
...
class Prediction {
  ...
  Prediction(
      ...
      this.isDeparture,
      this.blockId,
      this.affectedByLayover,
      this.dirTag,
      this.dirTitle,
      this.vehicle)
      : this.datetime = new DateTime.fromMillisecondsSinceEpoch(
            1000 * secondsSinceEpoch,
            isUtc: true);
}
